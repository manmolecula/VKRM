...
describe('useActivityLogLogic (Подход Options API + Composable)', () => {
  ...
  it('Сценарий 1: Успешная загрузка данных по запросу', async () => {
    ...
    const { entries, isLoading, isReady, handleSearch } = useActivityLogLogic()
  ...
  it('Сценарий 2: Обработка ошибки сети', async () => {
    ...
    const { error, isReady, handleSearch } = useActivityLogLogic()
  ...
  it('Сценарий 3: Вычисление диапазона пагинации (computed)', async () => {
    ...
    const { pagination, currentRange, handleChangePage } = useActivityLogLogic()
